...
Parser.prototype.eval = function() {
	...
	for (var i = 0; i < this.input.length; i++) {
		...
		if (c === '\n' && this.input[i-1] !== ',') {
			result = this.evaluateStatement(statement, varTable);
			statement = '';
		} else if (i === this.input.length - 1) {
			result = this.evaluateStatement(statement + c, varTable);
			statement = '';
		} else {
			statement += c;
		}
...
Parser.prototype.callFunc = function(func, varTable) {
	...
	for (var i = 0; i < func.parameters.length; i++) {
		...
		if (param.type === token.type.func) {
			param = this.callFunc(param, varTable);
		}

		func.parameters[i] = param = param.value;

		newVarTable[varName] = param;
	}

	var val = null;
	var functionToCall = this.fnDefinitions[func.value];

	if (isFunction(functionToCall)) {
		func.parameters.push(this);
		val = functionToCall.apply(null, func.parameters);
	} else {
		var p = new Parser(functionToCall.replace(',\n', '\n'));
		p.varTable = newVarTable;
		val = p.eval();
	}
