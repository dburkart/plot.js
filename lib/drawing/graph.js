...
Graph.prototype.addPoint = function(x, y) {
	if ( !this.x.locked && (this.x.min === null || x < this.x.min) ) this.x.min = x;
	if ( !this.x.locked && (this.x.max === null || x > this.x.max) ) this.x.max = x;
	if ( !this.y.locked && (this.y.min === null || y < this.y.min) ) this.y.min = y;
	if ( !this.y.locked && (this.y.max === null || y > this.y.max) ) this.y.max = y;
